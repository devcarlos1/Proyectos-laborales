<?php
namespace App\API\PaySera;

use App\API\PaySera\WebToPay\Exception\WebToPay_Exception_Validation;
use App\API\PaySera\WebToPay\WebToPay_Factory;
...
class WebToPay {
     ...
     * @throws WebToPayException on data validation error
     */
    public static function redirectToPayment($data, $exit = false): string {
        if (!isset($data['sign_password']) || !isset($data['projectid'])) {
            throw new WebToPayException('sign_password or projectid is not provided');
        }
        $password = $data['sign_password'];
